...
각 커넥션의 지연 시간을 겹치게 하고, 클라리언트의 인터넷 대역폭을 나눠서 사용한다면 웹페이지를 더 빠르게 내려받을 수 있다.
![](./4-12.png)

### 4.4.2 병렬 커넥션이 항상 더 빠르지는 않다.

- 클라이언트의 네트워크 대역폭이 좁을 때, 여러 개의 객체를 병렬로 내려받는 경우 성능상의 장점은 거의 없어진다.
...
- 요청이 완료된 후에도 TCP 커넥션을 유지하여 앞으로 있을 HTTP 요청에 재사용할 수 있다.
- 이미 맺어져 있는 커넥션은 TCP의 느린 시작으로 인한 지연을 피할 수 있다.
...
- 지속 커넥션은 병렬 커넥션과 함께 사용할 때에 가장 효과적이다.
- 지속 커넥션의 타입에는 HTTP/1.0+의 'keep-alive' 커넥션과 HTTP/1.1의 '지속' 커넥션이 있다.

### 4.5.2 HTTP/1.0+의 Keep-Alive 커넥션

- 상호 운영과 관련된 설계에 문제가 있다.
- 아직 많은 클라이언트와 서버는 이것을 사용하고 있다.
- 설계상의 문제점은 HTTP/1.1에서 수정되었다.
- 커넥션을 맺고 끊는 데 필요한 작업이 없어서 시간이 단축되었다.
  ![](./4-13.png)

### 4.5.3 Keep-Alive 동작

- keep-alive는 사용하지 않기로 결정되어 HTTP/1.1 명세에서 빠졋다.
- 아직 많이 사용되고 있기 때문에 HTTP 어플리케이션은 keep-alive 처리할 수 있게 개발해야 한다.
- 사용 과정
  - 클라이언트가 요청에 Connection:Keep-Alive 헤더를 포함한다.
  - 서버가 keep-alive를 수락할 경우, 응답 메시지에 같은 헤더를 포함한다.

![](./4-14.png)

### 4.5.4 Keep-Alive 옵션

- Keep-Alive 헤더는 요청일 뿐이며, 클라이언트나 서버가 무조건 따를 필요는 없다.
- keep-alive의 동작은 Keep-Alive 헤더의 쉼표로 구분된 옵션들로 제어할 수 있다.
  - timeout 파라미터 : 커넥션이 얼마간 유지될 것인지.
  - max 파라미터 : 커넥션이 몇개의 트랜잭션을 처리할 때까지 유지될 것인지.
  - 기타 진단이나 디버깅을 목적으로 하는 임의의 속성들
- Keep-Alive 헤더는 Connection:Keep-Alive 헤더가 있을 때만 사용할 수 있다.

### 4.5.5 Keep-Alive 커넥션 제한과 규칙

- keep-alive는 기본값이 아니다. 클라이언트는 Connection:Keep-Alive 헤더를 요청에 보내야 한다.
- 커넥션을 계속 유지하려면 모든 메시지에 Connection:Keep-Alive 헤더를 포함해야 한다.
- 클라이언트는 응답헤더에 Connection:Keep-Alive가 없는 것을 보고 서버가 응답후에 커넥션을 끊을 것임을 알 수 있다.
...
- 클라이언트는 응답을 모두 받기 전에 커넥션이 끊어졌을 경우, 별다른 문제가 없으면 요청을 다시 보낼 수 있게 준비되어 있어야 한다.

### 4.5.6 Keep-Alive와 멍청한(dumb) 프락시

- Connection 헤더의 무조건 전달
  - 프락시가 Connection 헤더를 이해하지 못해서 요청 그대로 다음 프락시에 전달한다.
  - 그림과 같은 상황이 발생할 수 있다.
    ![](./4-15.png)
- 프락시와 홉별 헤더
...
- 넷스케이프의 브러우저 및 프락시 개발자들은 프락시에 요청을 전달할 때, Connection 헤더를 Proxy-Connection로 대체하여 무시되도록 했다.
- 영리한 프락시는 의미 없는 Proxy-Connection 헤더를 Connection 헤더로 바꿈으로써 원하던 효과를 얻을 수 있다.
  ![](./4-16.png)
- 이러한 방식은 클라이언트와 서버 사이에 한 개의 프락시만 있는 경우에서만 동작한다.
  ![](./4-17.png)
- 문제가 발생하는 프락시들은 방화벽, 캐시 서버, 리버스 프락시 서버 가속기 등 네트워크 상에서 '보이지 않는' 경우가 많다.

### 4.5.8 HTTP/1.1의 지속 커넥션

- HTTP/1.1에서는 keep-alive 커넥션을 지원하지 않는 대신, 설계가 더 개선된 지속 커넥션을 지원한다.
- HTTP/1.1에서는 별도 설정을 하지 않는 한, 모든 커넥션을 지속 커넥션으로 취급한다.
- 애플리케이션은 트랜잭션이 끝난 다음 커넥션을 끊으려면 Connection:close 헤더를 명시해야 한다.
...
- 파이프라인의 제약사항
  ...
  - 클라이언트는 POST 요청같이 반복해서 보낼 경우 문제가 생기는 요청은 파이프라인을 통해 보내면 안 된다.
...
- 한 번 혹은 여러 번 실행해도 같은 결과를 반환한다면 그 트랜잭션은 멱등(idempotent)하다고 한다. (GET, HEAD, PUT, DELETE, TRACE, OPTIONS)
- POST와 같은 비멱등 메서드는 파이프라인을 통해 요청하면 안된다.
- 비멱등 요청을 다시 보내야 한다면, 이전 요청에 대한 응답을 받을 때까지 기다려야 한다.
- 비멱등 메서드나 순서에 대해 에이전트가 요청을 다시 보낼 수 있도록 기능을 제공한다 하더라도, 자동으로 재시도하면 안된다.
...
- 전체 끊기와 절반 끊기
  ...
  - 전체 끊기 : close()를 호출하여 입력 채널과 출력 채널을 모두 끊는 것
  - 절반 끊기 : shoutdown()을 호출하여 입력 채널이나 출력 채널 중 하나를 개별적으로 끊는 것.
- TCP 끊기와 리셋 에러
...
- 우아하게 커넥션 끊기
  - 자신의 출력 채널을 먼저 끊고 다른 쪽에 있는 기기의 출력 채널이 끊기는 것을 기다리는 것을 말한다.
  - 양쪽에서 더는 데이터를 전송하지 않을 것이라고 알려주면, 커넥션은 리셋 위험 없이 온전히 종료된다.
  - 상대방이 절반 끊기를 구현했다는 보장도 없고 절반 끊기를 했는지 검사해준다는 보장도 없다.
